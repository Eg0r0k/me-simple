...
- **Движение молчит, пока к нему не обратятся.** Постоянная анимация в системе одна —
  нажатие, `scale(.96)` за 100ms; hover только на десктопе. Появление экрана — единственное
  исключение, см. §3.14.
...
14. **Появление — один раз на элемент, и только на посадочной странице.** Разрешены `opacity`
    и `transform`, больше ничего. Длительность до 0.75s, ease `--ease-standard`, шаг стаггера
    0.05–0.06s, вход первого экрана укладывается в 1.2s. Заголовок может выезжать из маски
    по словам: слово в `overflow: hidden` с компенсацией выносных элементов, внутренняя часть
    идёт от `115%` к `0`.
    Секции ниже первого экрана получают по одному проявлению при попадании в кадр
    (`whileInView` + `inViewOptions: { once: true }`), 0.5s, сдвиг не больше 14px, и больше
    не повторяются. Никаких parallax, никакой анимации, привязанной к позиции скролла,
    никаких бесконечных циклов — исключение одно, дышащая точка статуса (`ds-breathe`).
    Задержки задавать числом: `computed`-ref, попавший в `delay`, ломает старт анимации молча.
    Всё это обязано выключаться на `prefers-reduced-motion`: у motion-v это `MotionConfig`
    с `reduced-motion="user"` в корне, потому что по умолчанию он игнорирует системную настройку.
...
- [ ] `prefers-reduced-motion` отключает press-scale, волну skeleton и анимацию появления.
- [ ] Появление живёт только на посадочной, только `opacity`/`transform`, вход ≤ 1.2s,
      секции ниже сгиба проявляются один раз (`once: true`).
