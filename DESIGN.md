...
14. **Появление экрана — один раз, на входе, и только на посадочном.** Разрешены `opacity`
    и `transform`, больше ничего. Длительность до 0.75s, ease `--ease-standard`, шаг стаггера
    0.05–0.06s, суммарно вход укладывается в 1.2s. Заголовок может выезжать из маски по словам:
    слово в `overflow: hidden` с компенсацией выносных элементов, внутренняя часть идёт от
    `115%` к `0`. Никакой анимации по скроллу, никаких parallax и бесконечных циклов —
    исключение одно, дышащая точка статуса (`ds-breathe`).
...
- [ ] Появление живёт только на посадочном экране, только `opacity`/`transform`, вход ≤ 1.2s.
