...
- Появление при открытии страницы: кнопка назад, шапка и карточки проявляются по образцу
  главной (`rise`: opacity 0→1, сдвиг 8px, 0.5s), карточки со стаггером 0.05s. Решение автора
  после просмотра: раньше правило 14 разрешало появления только на главной, теперь на всех
  страницах, по одному разу на элемент.
